...
5b. CHECK DEPOSIT CLASSIFICATION: Physical check deposits (descriptors starting with "CHECK" + number, or bare "DEPOSIT") require extra scrutiny. CHECK deposits over $25K → confidence 50 or below, is_excluded: true (likely owner capital, insurance, transfers). CHECK $5K-$25K → confidence 65, flag for review. CHECK under $5K → classify normally. Multiple large CHECKs ($50K+) in one month → exclude (capital infusions). Round-number CHECKs ($10K/$25K/$50K/$100K exactly) → strong indicator of capital transfer, exclude. Bare "DEPOSIT" entries → confidence 60. IMPORTANT: When in doubt about CHECK/DEPOSIT items, err on EXCLUDING (is_excluded: true) — better for user to manually include than to overstate revenue.
5c. PROTECTED REVENUE ACH — ALWAYS COUNT: "ROUTE"/"ROUTE COLLECTION"/"ROUTE PMT" → customer route collections = TRUE REVENUE. "CUSTOMER"/"CUST PMT" → customer payments = TRUE REVENUE. "VEND"/"VENDING" → vending income = TRUE REVENUE. Any ACH credit NOT matching a known MCA funder name → DEFAULT to ach_credit, is_excluded: false. The funder keywords should ONLY promote matching credits to LOAN type, never demote unrecognized customer ACH.
