...
export async function sendResendEmail({ from, to, subject, text }) {
 ...
 */
export async function sendNewDealEmail({ deal, isoName, totalEstimatedDebt, repInfo }) {
  ...
  if (repInfo) {
    const repName = [repInfo.first_name, repInfo.last_name].filter(Boolean).join(' ').trim();
    lines.push(
      '',
      'ASSIGNED REP',
      `${repName}${repInfo.title ? ` — ${repInfo.title}` : ''}`,
      repInfo.email || '',
      repInfo.phone || '',
      '',
      'Please direct all communication about this deal to the rep above.',
    );
  }

  lines.push('', 'Review in Analyzer: https://ff-analyzer.vercel.app');
