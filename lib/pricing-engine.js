...
/**
 * Calculate graduated ISO commission rate based on points
 * Points 1-5:   0.75% per point
 * Points 6-10:  1.00% per point
 * Points 11-15: 1.25% per point
 *
 * @param {number} points - Integer from 0 to 15
 * @returns {number} - Total commission rate as decimal (e.g., 0.0875 for 10 points)
 */
...
export function calculateTierAllocations(positions, tadAt100, tierPcts, opts = {}) {
  ...
  const totalUnlockedWeekly = unlocked.reduce((sum, p) => sum + p._totalWeekly, 0);
  ...
  if (enforcementWeighting && Object.keys(scoreMap).length > 0) {
    ...
    unlockedShares = unlocked.map(p => {
      ...
      const baseShare = totalUnlockedWeekly > 0 ? p._totalWeekly / totalUnlockedWeekly : 0;
    ...
    unlockedShares = unlocked.map(p => ({
      position: p,
      adjustedShare: totalUnlockedWeekly > 0 ? p._totalWeekly / totalUnlockedWeekly : 0,
    }));
  }
